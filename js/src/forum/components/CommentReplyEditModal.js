...
export default class CommentReplyEditModal extends Modal {
  postId = null;
  commentId = null;
  isLoading = false;
  constructor(vnode) {
...
if(flag){    const formData = this.commentFor
    ...
    if (content.trim()) {
      ...
      fetch(`${app.forum.attribute('apiUrl')}/${this.commentFor ? `wikicomment/${this.postId.id}` : `/wikireplycomment/${this.postId.id}`}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="Modal-body">
        <div className="Form">
          ...
          <div className="Form-group">
            {this.isLoading ? (
              <div class="parent_loader"><div className="loader">Loading...</div></div> // Display the loader
            ) : (
              <button type="button" onclick={this.handleSubmit.bind(this)} className="Button Button--primary" disabled={this.isLoading}>
                Add
              </button>
            )}
          </div>
