...
export function AllSubCategories() {
    ...
    $(document).ready(async function() {
        ...
        $('.sid_category_btn,.sid_view_point_btn').removeClass('active');
        $(".sid_subcategory_btn ").addClass('active');

        if (tableContainer) {
            const csrfToken = await fetchCsrfToken();
            fetch(`${url}/wiki/subcategory`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
                ...
                function displayDataForPage() {
                    ...
                    const rowsHTML = dataToDisplay.map(item => `
                        <tr>
                            ...
                            <td>${item.name}</td>
                            <td>
                                <button id="${item.id}" class='Button subcategory_table_edit_btn'>Edit</button>
                            </td>
                        </tr>
                    `).join('');
